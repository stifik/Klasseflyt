/**
 * Clean WebSocket hook implementation (migrated from useNFCWebSocket2.ts)
 */
...
export function useNFCWebSocket({
    ...
    try {
      ...
      switch (data.type) {
        case 'status':
          ...
          setCurrentReader(data.currentReader || null);
          setIsMonitoring(!!data.isMonitoring);
          console.log('📊 Status updated - readers:', data.readersConnected, 'monitoring:', data.isMonitoring);
          break;

        case 'card_detected':
          console.log('🎴 Card detected event:', data.uid);
          if (data.uid && data.cardId && onCardDetected) {
            onCardDetected({
              uid: data.uid,
              cardId: data.cardId,
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      ws.onopen = () => {
        ...
        // Use the desired monitoring ref to decide whether to restart monitoring
        if (autoMonitor || isMonitoringRef.current) {
          console.log('🔄 Restarting monitoring after reconnect');
          setTimeout(() => {
            if (wsRef.current?.readyState === WebSocket.OPEN) {
              wsRef.current.send(JSON.stringify({ command: 'start_monitoring' }));
  ...
  useEffect(() => {
    ...
    return () => {
      // mark as manual to avoid reconnection attempts from this instance
      manualDisconnectRef.current = true;
      disconnect();
    };
