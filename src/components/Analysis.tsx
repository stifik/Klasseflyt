...
import type { Student, Subject, Homework, Submission, DailyCheck, HomeworkStatus, Remark, ReportSettings, DashboardSubTab } from '@/lib/types';
...
const WeeklySummary = ({ students, subjects, homework, submissions, remarks, settings }: Omit<AnalysisProps, 'activeSubTab'>) => {
    ...
    return (
        <Card>
            ...
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                ...
                <Button onClick={handleGenerateSummaries} disabled={isGenerating || dailyChecks === undefined} className="w-full sm:w-auto">
...
const useStudentStats = (students: Student[], subjects: Subject[], homework: Homework[], submissions: Submission[], remarks: Remark[]) => {
    const dailyChecks = useLiveQuery(() => db.dailyChecks.toArray(), [], []);
    
    return React.useMemo(() => {
        if (dailyChecks === undefined) return [];
    ...
    }, [students, subjects, homework, submissions, remarks, dailyChecks]);
};


const StudentReport = ({ students, subjects, homework, submissions, remarks }: Omit<AnalysisProps, 'settings' | 'activeSubTab'>) => {
    const [openStudents, setOpenStudents] = useState<Record<string, boolean>>({});
    const studentStats = useStudentStats(students, subjects, homework, submissions, remarks);
    ...
    return (
        <Card>
            ...
            <CardContent className="space-y-4 screen-only">
                 {studentStats.length === 0 && <div className="text-center p-8 text-muted-foreground">Laster data...</div>}
...
export default function Analysis(props: AnalysisProps) {
    const [activeTab, setActiveTab] = useState<DashboardSubTab>(props.activeSubTab || "weekly-summary");
    ...
    return (
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as DashboardSubTab)} className="w-full space-y-4">
            <TabsList className="grid w-full grid-cols-3 no-print">
                <TabsTrigger value="weekly-summary">Ukesoppsummering</TabsTrigger>
                <TabsTrigger value="student-reports">Elevrapporter</TabsTrigger>
                <TabsTrigger value="remark-analysis">Anmerkningsanalyse</TabsTrigger>
            </TabsList>
            ...
            <TabsContent value="remark-analysis">
